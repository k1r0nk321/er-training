...
【Step1で研修医に事前提示した情報（問診評価の除外対象）】
以下の情報はStep1で研修医に既に提示済みです。これらの内容を研修医が「問診で確認した」としても、問診の加点対象にはなりません。問診の評価は、Step1提示情報以外で研修医が追加的に引き出した情報（症状の詳細・経過・既往歴の深掘り・社会歴・家族歴など）に対して行ってください。
- バイタルサイン：${caseData.vital_signs || ''}
- 主訴：${caseData.chief_complaint || ''}
- 現病歴・既往歴・背景情報：${caseData.history || ''}
- 初期検査所見：${caseData.initial_labs || ''}
...
以下のJSON形式のみで返答（マークダウン記号不要）：
{
  ...
  "interview_feedback": "問診の質1〜2文（Step1提示情報の確認は評価対象外。Step2で追加的に引き出した情報を評価）",
