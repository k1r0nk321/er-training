...
現病歴：${caseData.history || ''}
【指示】
- 疾患名・診断名は絶対に言わないこと（感染性心内膜炎・心筋梗塞・肺塞栓などの具体的な疾患名は一切禁止）
- 検査・治療・処置に関するヒントは一切含めないこと
- 「この症例で意識すべき鑑別の視点」を2〜3点、臨床的な思考の切り口（症状の組み合わせ・リスク因子・病態の方向性）のみで示すこと
- 例：「発熱＋心雑音の組み合わせが示す病態の方向性」「抜歯後に生じた症状が示唆する感染経路の考え方」のように、思考の視点を示すに留める
- 「must not miss」の観点は臓器カテゴリ・病態カテゴリのみで表現すること（例：「循環器系の緊急性」「神経系への波及」）
- 100字以内でコンパクトに`;
    try {
      const response = await fetch('/api/score', {
        method: 'POST',
...
- 【重要：整合性の確保】すでに生成済みの検査結果がある場合は、その数値・所見と矛盾しない結果を生成すること。例えばHbが10.0 g/dLと生成済みなら新たな検査でもHbは同じ値にする
【すでに生成済みの検査結果（これらと矛盾しない結果を生成すること）】
${Object.keys(examResults).length > 0
  ? Object.entries(examResults).map(([k, v]) => `${k}：${v}`).join('\n')
  : '（なし）'}
